...
class SignUpHandler(EnhancedHandler):
    ...
    def post(self):
        ...
        user_name = self.request.get('user_name_signup_name')
        ...
        if not valid_inputs.valid_password(password):
            ...
            has_error = True
        if not valid_inputs.valid_email(email):
        ...
        if has_error:
            ...
            if User().by_user_name(user_name):
                self.arg_dict['error_user_name'] = "That user already exists."
                self.render('signup_form.html', **self.arg_dict)
            else:
                pw_hash = SecureValue().make_pw_hash(user_name, password)
                User().register_user(user_name, pw_hash, email)
